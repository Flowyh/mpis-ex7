    ...
    for nth in tqdm(range(self.n_beg, self.n_end + 1), ncols=140, desc=f'Task 1 N=[{self.n_beg * self.step}, {self.n_end * self.step}] K={self.k} Step={self.step} D={self.d}'):
      ...
      for kth in range(self.k):
        ...
        while(balls != n):
          ...
          temp_min = list(np.where(temp == np.min(temp))[0])
          random_min = balls % len(temp_min)
          # print(random_urns, temp, temp_min, random_min, random_urns[temp_min[random_min]])
          # Put ball into the random minimal urn
          self.urns[random_urns[temp_min[random_min]]] = self.urns[random_urns[temp_min[random_min]]] + 1
        # Ln
